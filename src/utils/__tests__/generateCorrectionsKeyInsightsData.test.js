...
// =============================================================================
import generateCorrectionsKeyInsightsData from "../generateCorrectionsKeyInsightsData";

describe("generateCorrectionsKeyInsightsData.js", () => {
  const mockData = {
  ...
  const correctionsKeyInsightsData = generateCorrectionsKeyInsightsData(mockData);

  it("should produce card data", () => {
    expect(correctionsKeyInsightsData[0]).toMatchObject({
      title: "Prison Population",
      number: 1223,
      percentChange: -31,
    });
    expect(correctionsKeyInsightsData[1]).toMatchObject({
      title: "Parole Revocations",
      number: 137,
      percentChange: -20,
    });
    expect(correctionsKeyInsightsData[2]).toMatchObject({
      title: "Probation Revocations",
      number: 23,
      percentChange: -31,
    });
